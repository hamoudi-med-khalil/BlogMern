...
const SinglePostActions = ({ post }) => {
    const { user } = useUser()
    const { getToken } = useAuth()
    const navigate = useNavigate()

    const { isPending, data: savedPosts, error } = useQuery({
        queryKey: ['savedPosts'],
        queryFn: async () => {
            ...
            return response.data; // ✅ Retourne uniquement le tableau des posts
        },
    ...
    return (
        <div className='flex flex-col gap-4'>
            ...
            {
                isAdmin && <div className="flex items-center gap-2 py-2 text-sm cursor-pointer" onClick={handelFeature}>
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 48 48"
                        width="20px"
...

            {
                user && (post.user.username == user.username || isAdmin) && (
                    <div className="flex items-center gap-2 py-2 text-sm cursor-pointer" onClick={handleDelete}>
                        ...
                        {deleteMutation.isPending && <span>'(prpgress)'</span>}
                    </div>
                )
            }
